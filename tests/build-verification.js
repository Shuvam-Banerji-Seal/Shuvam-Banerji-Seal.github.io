...
test("CSS files are minified (no excessive whitespace)", () => {
  ...
  for (const cssFile of cssFiles) {
    ...
    if (lines.length > 50 && content.length > 1000) {
      ...
      assert(
        ratio > 50,
        `${cssFile.replace(ROOT + "/", "")}: CSS may not be properly minified (${ratio.toFixed(0)} chars/line)`,
      );
    }
  }
});
...
test("JS files are minified", () => {
  ...
  for (const jsFile of jsFiles) {
    ...
    if (content.length > 5000) {
      ...
      assert(
        ratio > 30,
        `${jsFile.replace(ROOT + "/", "")}: JS may not be properly minified`,
      );
    }
  }
});
...
test("dist/index.html has DOCTYPE", () => {
  assert(
    distIndex.includes("<!doctype html>") ||
      distIndex.includes("<!DOCTYPE html>"),
    "Missing DOCTYPE",
  );
});
...
test("dist/index.html has theme script", () => {
  assert(
    distIndex.includes("data-theme") ||
      distIndex.includes("localStorage.getItem"),
...
test("dist/index.html has loading screen", () => {
  assert(
    distIndex.includes("loading-screen") || distIndex.includes("loading-bar"),
...
test("dist/index.html has navbar container", () => {
  assert(
    distIndex.includes("navbar-container") || distIndex.includes("navbar"),
...
test("dist/index.html has hero terminal", () => {
  assert(
    distIndex.includes("terminal") || distIndex.includes("hero"),
...
test("dist/index.html has contact form", () => {
  assert(
    distIndex.includes("contact") || distIndex.includes("form"),
...
test("main.css uses CSS variables (not hardcoded colors)", () => {
  ...
  for (const pattern of hardcodedPatterns) {
    ...
    if (matches) {
      assert(
        matches.length < 3,
        `main.css has ${matches.length} hardcoded light theme colors`,
      );
    }
  }
});
...
test("navbar.js exports or initializes", () => {
  ...
  assert(
    navbarJS.includes("initThemeToggle") ||
      navbarJS.includes("DOMContentLoaded"),
...
test("main.js has theme toggle logic", () => {
  ...
  assert(
    mainJS.includes("theme") || mainJS.includes("Theme"),
...
test("audio-studio.js has AudioContext check", () => {
  ...
  assert(
    audioJS.includes("AudioContext") || audioJS.includes("webkitAudioContext"),
...
test("audio-studio.js has recording functionality", () => {
  const audioJS = readFileSync(join(ROOT, "assets/js/audio-studio.js"), "utf8");
  assert(
    audioJS.includes("MediaRecorder") || audioJS.includes("getUserMedia"),
...
const audioHTML = readFileSync(
  join(ROOT, "pages/tools/audio-studio.html"),
  "utf8",
);
const audioCSS = readFileSync(
  join(ROOT, "assets/css/audio-studio.css"),
  "utf8",
);
const audioJS = readFileSync(join(ROOT, "assets/js/audio-studio.js"), "utf8");
...
test("Audio Studio has effects rack", () => {
  assert(
    audioHTML.includes("effects-rack") || audioHTML.includes("effect-panel"),
...
test("Audio Studio has mixer panel", () => {
  assert(
    audioHTML.includes("mixer-panel") || audioHTML.includes("mixer"),
    "Missing mixer panel",
  );
});
...
test("Audio Studio CSS has no duplicate selectors", () => {
  ...
  if (duplicates.length > 0) {
    assert(
      false,
      `Duplicate compound selectors: ${duplicates.slice(0, 3).join(", ")}`,
    );
  }
});
...
const ytHTML = readFileSync(
  join(ROOT, "pages/tools/youtube-downloader.html"),
  "utf8",
);

test("YouTube Downloader has URL input", () => {
  assert(
    ytHTML.includes("video-url") || ytHTML.includes("youtube-url"),
...
test("YouTube Downloader uses noembed for video info", () => {
  assert(
    ytHTML.includes("noembed.com"),
    "Should use noembed.com for video info",
  );
});

test("YouTube Downloader has download service links", () => {
  assert(
    ytHTML.includes("y2mate.com") ||
      ytHTML.includes("savefrom.net") ||
      ytHTML.includes("cobalt.tools"),
...
test("YouTube Downloader has proper error handling", () => {
  assert(
    ytHTML.includes("catch") || ytHTML.includes("error"),
...
test("PDF Studio has compress functionality", () => {
  assert(
    pdfHTML.includes("compress") || pdfHTML.includes("Compress"),
...
test("PDF Studio has merge functionality", () => {
  assert(
    pdfHTML.includes("merge") || pdfHTML.includes("Merge"),
...
test("PDF Studio has split functionality", () => {
  assert(
    pdfHTML.includes("split") || pdfHTML.includes("Split"),
...
test("PDF Studio has rotate functionality", () => {
  assert(
    pdfHTML.includes("rotate") || pdfHTML.includes("Rotate"),
...
test("PDF Studio has watermark functionality", () => {
  assert(
    pdfHTML.includes("watermark") || pdfHTML.includes("Watermark"),
...
test("Total CSS size < 500KB", () => {
  ...
  assert(
    totalSize < 500 * 1024,
    `Total CSS size ${(totalSize / 1024).toFixed(0)}KB exceeds 500KB`,
  );
});
...
test("Total JS size < 5MB", () => {
  ...
  assert(
    ...
    `Total JS size ${(totalSize / 1024 / 1024).toFixed(1)}MB exceeds 5MB`,
  );
});

test("No single CSS file > 200KB raw / 30KB gzipped", () => {
  // The index-page bundle intentionally aggregates theme+main+mobile+overrides
  // into ONE ordered file (cascade-order guarantee, see assets/css/index-page.css).
  // Raw bytes crossed 100KB there by design; the meaningful performance metric
  // is wire size, so assert on gzip (GitHub Pages serves gzip/brotli) while
  // keeping a raw ceiling against un-compressible bloat.
  const cssFiles = getAllFiles(join(DIST, "assets"), ".css");
  for (const f of cssFiles) {
    ...
    assert(
      size < 200 * 1024,
      `${f.replace(ROOT + "/", "")} is ${(size / 1024).toFixed(0)}KB raw`,
    );
    assert(
      gz < 30 * 1024,
      `${f.replace(ROOT + "/", "")} is ${(gz / 1024).toFixed(0)}KB gzipped`,
    );
  }
});
