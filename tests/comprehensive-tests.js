...
for (const cssFile of cssFiles) {
  ...
  test(`CSS braces balanced: ${relPath}`, () => {
    ...
    assert.strictEqual(
      opens,
      closes,
      `Unbalanced braces: ${opens} opening vs ${closes} closing`,
    );
  });
  ...
  test(`CSS no empty rules: ${relPath}`, () => {
    ...
    if (emptyRules && emptyRules.length > 0) {
      ...
      assert(
        emptyRules.length < 5,
        `Too many empty CSS rules: ${emptyRules.length}`,
      );
    }
  });
}
...
test("theme.css has all dark theme definitions", () => {
  ...
  for (const theme of themes) {
    assert(
      themeCSS.includes(`data-theme="${theme}"`),
...
test("theme.css has light theme definition", () => {
  const themeCSS = readFileSync(join(ROOT, "assets/css/theme.css"), "utf8");
  assert(
    themeCSS.includes(":root") || themeCSS.includes('data-theme="light"'),
...
test("All themes have required CSS variables", () => {
  ...
  const darkThemeMatch = themeCSS.match(
    /html\[data-theme="dark"\][^}]+\{[^}]+\}/s,
  );
  if (darkThemeMatch) {
    for (const v of requiredVars) {
      assert(
        darkThemeMatch[0].includes(v + ":"),
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        ...
        if (
          !line.includes(":root") &&
          !line.includes(":not(") &&
          !line.includes("data-theme")
        ) {
          warn(
            `index.html line ${i + 1} has hardcoded color ${color}: ${line.substring(0, 80)}`,
          );
        }
      }
    }
...
for (const htmlFile of htmlFiles) {
  ...
  test(`Images have alt text: ${relPath}`, () => {
    ...
    for (const img of imgTags) {
      if (!img.includes("alt=")) {
        warn(
          `${relPath}: Image missing alt attribute: ${img.substring(0, 60)}`,
        );
      }
    }
  });
    ...
    for (const btn of buttons) {
      if (
        ...
        if (btn.includes("data-lucide") && !btn.includes("title=")) {
          warn(
            `${relPath}: Icon button missing title/aria-label: ${btn.substring(0, 60)}`,
          );
        }
      }
    }
...
test("No inline styles > 1000 chars", () => {
  for (const htmlFile of htmlFiles) {
    ...
    if (inlineStyles) {
      warn(
        `${htmlFile.replace(ROOT + "/", "")}: Has ${inlineStyles.length} very long inline styles`,
      );
    }
  }
});
...
test("CSS files not too large", () => {
  for (const cssFile of cssFiles) {
    ...
    if (sizeKB > 100) {
      warn(
        `${cssFile.replace(ROOT + "/", "")}: Large CSS file (${sizeKB.toFixed(1)}KB)`,
      );
    }
  }
});
...
test("navbar.js exists and has required functions", () => {
  ...
  assert(
    content.includes("initThemeToggle") || content.includes("theme"),
    "navbar.js missing theme toggle functionality",
  );
});

test("air-guitar.js binds the camera video element (send-gate dependency)", () => {
  // Regression guard: air-guitar.js receives only the MediaStream from
  // webcam-tester.js; its rAF loop gates hands.send() on this.video.
  // If this.video is never assigned, tracking silently never runs
  // (status stuck at "Waiting for hand…" — bug found 2026-08-23).
  const src = readFileSync(join(ROOT, "assets/js/air-guitar.js"), "utf8");
  assert(
    src.includes('getElementById("cam")'),
    "air-guitar.js must acquire the #cam video element — without it hands.send() never fires",
  );
  assert(/this\.video\s*=/, "air-guitar.js must assign this.video");
});

test("webcam-tester.js attaches the stream for air guitar", () => {
  const src = readFileSync(join(ROOT, "assets/js/webcam-tester.js"), "utf8");
  assert(
    src.includes("AirGuitar?.attach") || src.includes("AirGuitar.attach"),
    "webcam-tester.js must call AirGuitar.attach() after getUserMedia",
  );
});
