...
// Mermaid is a large third-party engine (~600 kB minified). Load it on demand
// so the editor shell paints before the engine downloads, and so the bundler
// emits it as its own lazy chunk instead of inflating the page entry chunk.
// One shared promise keeps every caller on the same instance.
...
function Preview({ code, theme, diagramTheme = "default", onError }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      ...
      .catch(() => {
        /* engine unavailable — the render effect reports the failure */
      });
    return () => {
      cancelled = true;
    };
  ...
  return (
    <div className="preview-container">
      ...
      <div className="zoom-controls">
        ...
        <button
          onClick={() => setScale((s) => Math.min(s, 1) || 1)}
          title="Fit to view"
        >
          <svg
            width="16"
