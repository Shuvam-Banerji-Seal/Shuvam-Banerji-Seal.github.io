/**
 * Safely parse a JSON array from localStorage. Corrupt or tampered
 * storage must never crash the player (falls back to an empty array).
 * @param {string} key
 * @returns {Array}
 */
function safeParseArray(key) {
  try {
    ...
    console.warn(
      `[MusicPlayer] Corrupt localStorage entry "${key}", resetting`,
      e,
    );
    try {
...
class MusicPlayer {
  constructor() {
    ...
    this.state = {
      ...
      currentFolder: null,
      volume: 1.0,
      favorites: safeParseArray("music_favorites"),
      playlists: safeParseArray("music_playlists"),
    };
  ...
  toggleFavorite(track) {
    ...
    // Prefer the currently playing track for the now-playing UI; fall back
    // to the toggled track so this never crashes when nothing is playing.
    this.updateUI(this.state.playlist[this.state.currentIndex] || track);

    // Dispatch event for UI updates
    document.dispatchEvent(
      new CustomEvent("favoritesUpdated", { detail: this.state.favorites }),
    );
  }
  ...
  createPlaylist(name) {
    ...
    document.dispatchEvent(
      new CustomEvent("playlistsUpdated", { detail: this.state.playlists }),
    );
  }
    ...
    try {
      ...
      if (this.audio) {
        if (!this.source) {
          ...
          this.log(
            "Audio chain connected: Source -> 10 Filters -> Analyser -> Destination",
          );
        }
      }

      this.log(
        `AudioContext initialized: ${this.audioCtx.state}, filters: ${this.filters.length}`,
      );
    } catch (e) {
  ...
  setEqualizer(bandIndex, value) {
    ...
    if (dbDisplay) {
      ...
      dbDisplay.className =
        "db-value " + (val > 0 ? "positive" : val < 0 ? "negative" : "");
    }

    // Save to local storage
    localStorage.setItem(
      "music_eq_values",
      JSON.stringify(this.state.eqValues),
    );
  }
  ...
  applyPreset(presetName) {
    ...
    values.forEach((val, index) => {
      ...
      const slider = document.querySelector(
        `input[data-band="${this.eqBands[index]}"]`,
      );
  ...
  async loadLibrary() {
    ...
    const JSDELIVR_URL =
      "https://cdn.jsdelivr.net/gh/Shuvam-Banerji-Seal/Shuvam-Banerji-Seal.github.io@gh-pages/music-library.json";
    ...
    try {
      ...
      document.dispatchEvent(
        new CustomEvent("libraryLoaded", { detail: data }),
      );
  ...
  async playTrack(index) {
    ...
    const isProduction =
      ...
      window.location.hostname.includes("shuvam-banerji-seal.github.io");
    let src = track.file;
    ...
    if (isProduction) {
      if (track.cdnUrl) {
        ...
        this.error(
          "CDN URL missing for track in production mode. Attempting to construct fallback...",
        );
        ...
        const relativePath = track.file.replace(
          /^\/assets_for_my_website\//,
          "",
        );
  ...
  toggleShuffle() {
    ...
    if (btn) {
      btn.setAttribute(
        "data-tooltip",
        `Shuffle: ${this.state.shuffle ? "On" : "Off"}`,
      );
  ...
  toggleRepeat() {
    ...
    if (btn) {
      btn.setAttribute(
        "data-tooltip",
        `Repeat: ${modeLabels[this.state.repeatMode]}`,
      );
      btn.classList.add("pulse");
      setTimeout(() => btn.classList.remove("pulse"), 400);
    }
  ...
  playNextInFolder() {
    ...
    if (nextIndex !== -1) {
      ...
      const firstInFolder = this.state.playlist.findIndex(
        (t) => t.folder === this.state.currentFolder,
      );
      if (firstInFolder !== -1) {
  ...
  updateUI(track) {
    if (!track) return; // nothing selected yet — nothing to render
...
window.toggleFavorite = () => {
  const track =
    window.musicPlayer.state.playlist[window.musicPlayer.state.currentIndex];
  if (track) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.body.addEventListener(
    ...
    () => {
      if (
        window.musicPlayer.audioCtx &&
        window.musicPlayer.audioCtx.state === "suspended"
      ) {
  ...
  function drawVisualizer() {
    ...
    if (
      ...
      document.getElementById("stat-ctx").textContent =
        window.musicPlayer.audioCtx.state;
      document.getElementById("stat-sr").textContent =
        window.musicPlayer.audioCtx.sampleRate;
      document.getElementById("stat-ch").textContent =
        window.musicPlayer.analyser.channelCount;
      document.getElementById("stat-buf").textContent =
        window.musicPlayer.analyser.fftSize;
      document.getElementById("stat-time").textContent =
        window.musicPlayer.audio.currentTime.toFixed(3);
      document.getElementById("stat-vol").textContent =
        Math.round(window.musicPlayer.audio.volume * 100) + "%";
      document.getElementById("stat-src").textContent =
        window.musicPlayer.audio.src;
    }
  }
...
const PlaylistManager = {
  ...
  addTrack(playlistId, track) {
    ...
    if (playlist) {
      ...
      const exists = playlist.tracks.some(
        (t) => t.file === track.file && t.folder === track.folder,
      );
      if (!exists) {
  ...
  getStats() {
    return (
      JSON.parse(localStorage.getItem(this.STATS_KEY)) || {
        tracksPlayed: 0,
        totalSeconds: 0,
      }
    );
...
const originalPlayTrack = window.musicPlayer?.playTrack?.bind(
  window.musicPlayer,
);
...
window.showAddToPlaylistModal = (trackIndex) => {
  ...
  if (playlists.length === 0) {
    list.innerHTML =
      '<div class="empty-playlists">No playlists. Create one first!</div>';
  } else {
