...
class BenzeneLoader {
  ...
  animate(timestamp) {
    ...
    this.bonds.forEach((bond) => {
      bond.progress = Math.max(
        0,
        Math.min(1, (this.animationProgress - bond.targetProgress + 0.2) / 0.2),
      );
    });
  ...
  start() {
    // Guard: constructor bails out early when the target container is
    // missing, leaving this.canvas undefined. Starting the animation
    // anyway would throw on the first frame.
